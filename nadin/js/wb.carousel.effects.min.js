/* jQuery Carousel Effects Plugin for WYSIWYG Web Builder 16 - Copyright Pablo Software solutions 2020 - http://www.wysiwygwebbuilder.com */
...
b('<div class="pagination" />');d=b(c).find(".frame").get();for(var h=0;h<d.length;h++){var g=b("<a />").attr("id",String(h+1)).css("cursor","pointer");b("<img />").attr({id:"page_"+String(h+1),src:a.pagination_img_default}).appendTo(g);g.click(function(){b.carouseleffects.gotoFrame(c,b(this).attr("id"))});g.appendTo($pagination)}$pagination.appendTo(b(c).parent());b.carouseleffects.updateNavigation(c,0,e.last)}if(1==e.scalemode||3==e.scalemode)b.carouseleffects.resize(c),b(window).resize(function(){b.carouseleffects.resize(c)});
...
1?(--state.current,state.last=state.current+1):(state.current=state.frames.length-1,state.last=0);b.carouseleffects.frameTransition(c)};b.carouseleffects.next=function(c){if("sequence"==c.type)c.current+1<c.frames.length?(c.current+=1,c.last=c.current-1):(c.current=0,c.last=c.frames.length-1);else if("random"==c.type)for(c.last=c.current;c.current==c.last;)c.current=Math.floor(Math.random()*c.frames.length)};b.carouseleffects.frameTransition=function(c){var a=b(c).data("state"),d=a.current;if(a.random){var e=
"fade slide puff blind drop clip shuffle blast blocks rain swirl squares bars".split(" "),h=["left","up","down","right"],g=["horizontal","vertical"];a.mode=e[Math.floor(Math.random()*e.length)];if("slide"==a.mode||"drop"==a.mode)a.direction=h[Math.floor(Math.random()*h.length)];if("clip"==a.mode||"blinds"==a.mode||"bars"==a.mode)a.direction=g[Math.floor(Math.random()*g.length)]}for(e=0;e<a.frames.length;e++)b(a.frames[e]).css("display","none");b(a.frames[a.last]).css("display","block").css("zIndex",
